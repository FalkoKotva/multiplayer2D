...
func state_normal():
	if is_multiplayer_authority():
		...
		if attack_cooldown_timer.is_stopped() && global_position.distance_to(target_position) < 155:
...
func state_charge_attack():
	if is_multiplayer_authority():
		...
		if charged_attack_timer.is_stopped():
...
func enter_state_attack():
	if is_multiplayer_authority():
		collision_mask = 1 << 0 # bit mask shift Bit 0 value 1
